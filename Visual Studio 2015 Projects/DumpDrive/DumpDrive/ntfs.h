...
static PNTFS_ATTR_CONTEXT PrepareAttributeContext(PNTFS_ATTR_RECORD AttrRecord)
{
    ...
    Context = (PNTFS_ATTR_CONTEXT)malloc(FIELD_OFFSET(NTFS_ATTR_CONTEXT, Record) + AttrRecord->Length); //FrLdrTempAlloc(FIELD_OFFSET(NTFS_ATTR_CONTEXT, Record) + AttrRecord->Length,        TAG_NTFS_CONTEXT);

    memcpy(&Context->Record, AttrRecord, AttrRecord->Length);
